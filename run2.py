...
def strat2(n, k, array, print_result=True):
    # print(array)
    queue = PriorityQueue()
    for i, arr in enumerate(array):
        queue.put((arr[0]/arr[1], i, arr)) # put element according to numWorking/total
    ...
    for i in range(0, k):   # put k devices one by one
        ...
        bag[0] += 1 # numWorking
        bag[1] += 1 # total
        queue.put((bag[0]/bag[1], index, bag)) # change values and put it back to queue
        result += "%d "%(index)
